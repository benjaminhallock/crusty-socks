...
const Admin = ({ user }) => {
      ...
      switch (type) {
        case 'user':
          ...
          if (result.success) {
            setData((prev) => ({
              ...prev,
              users: prev.users.map((user) => (user._id === id ? result.user : user)),
            }))
          } else {
  ...
  return (
    <div className='flex flex-col md:flex-row h-screen bg-white dark:bg-gray-900 relative'>
      <div className='md:hidden flex items-center justify-between bg-white dark:bg-gray-800 p-4 border-b border-gray-200 dark:border-gray-700'>
        <h1 className='text-gray-900 dark:text-white text-xl font-bold'>Admin Portal</h1>
        <button
          onClick={toggleSidebar}
          className='text-gray-900 dark:text-white p-2 rounded hover:bg-gray-100 dark:hover:bg-gray-700 focus:outline-none'
        >
          <svg
      ...
      <div
        ...
        } md:block w-full md:w-64 bg-white dark:bg-gray-800 p-4 md:h-screen fixed md:static z-10 inset-0 md:inset-auto overflow-y-auto`}
      >
        <h1 className='text-gray-900 dark:text-white text-2xl font-bold mb-8 hidden md:block'>
          Admin Portal
        </h1>
        <nav>
      ...
      <div className='flex-1 p-4 md:p-8 overflow-auto w-full'>
        ...
        {error && (
          <div className='bg-red-100 dark:bg-red-900 text-red-900 dark:text-red-100 p-3 rounded mb-4'>
            {error}
          </div>
        )}

        {successMessage && (
          <div className='bg-green-100 dark:bg-green-900 text-green-900 dark:text-green-100 p-3 rounded mb-4'>
            {successMessage}
          </div>
        )}
        ...
        <div className='mb-6'>
          <input
            ...
            className='w-full p-2 rounded bg-white dark:bg-gray-700 text-gray-900 dark:text-white border border-gray-300 dark:border-gray-600 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500'
          />
        </div>
                ...
                <tr className='border-b border-gray-200 dark:border-gray-700'>
                  {[
                    ...
                    <th
                      ...
                      className='px-3 md:px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-600'
                    >
                      {header}
        ...
        {/* Lobbies Table with Responsive Design */}
        {activeTab === 'lobbies' && (
          <div className='bg-white dark:bg-gray-800 rounded-lg shadow overflow-x-auto'>
            <table className='min-w-full'>
              <thead className='bg-gray-50 dark:bg-gray-700'>
                      ...
                      <span
                        className={`px-2 py-1 rounded-full text-xs ${
                          lobby.gameState === 'WAITING'
                            ...
                            : lobby.gameState === 'CHOOSING_WORD'
                            ? 'bg-yellow-500 text-white'
                            : lobby.gameState === 'ROUND_ENDED'
                            ? 'bg-purple-500 text-white'
                            : lobby.gameState === 'GAME_ENDED'
