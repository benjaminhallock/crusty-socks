...
const Admin = () => {
	...
	const [activeView, setActiveView] = useState('users') // Changed from 'overview'
		...
		return (
			<div className='flex justify-between items-center px-6 py-4 bg-gray-50 dark:bg-gray-900/50'>
				<div className='text-sm text-gray-700 dark:text-gray-300'>
					...
					{Math.min(currentPage * itemsPerPage, totalItems.length)} of {totalItems.length} entries
				</div>
	...
	useEffect(() => {
		const fetchData = async () => {
			setLoading(true)
			try {
				const [usersResponse, lobbiesResponse, reportsResponse, chatsResponse] = await Promise.all([
					getAllUsers(),
					getAllLobbies(),
					getAllReports(),
					getAllChats(),
				...
				setData({
					...
					chats: chatsResponse.success ? chatsResponse.chats : [], // Initialize with actual chats
				})
				...
				if (!chatsResponse.success) errors.push(`Chats: ${chatsResponse.error}`)
	...
	const handleUpdateStatus = async (reportId, newStatus) => {
		try {
			...
			setError('An error occurred while updating report status: ' + error.message)
		}
	}
			...
			switch (type) {
				case 'user':
					...
					if (result.success) {
						setData(prev => ({
							...prev,
							users: prev.users.map(user => (user._id === userId ? result.user : user)),
						}))
					} else {
				...
				case 'lobby':
					...
					if (result.success) {
						setData(prev => ({
							...prev,
							lobbies: prev.lobbies.map(lobby => (lobby._id === id ? result.lobby : lobby)),
						}))
					}
					break
				case 'report':
					...
					if (result.success) {
						setData(prev => ({
							...prev,
							reports: prev.reports.map(report => (report._id === id ? result.report : report)),
						}))
					}
					break
				default:
				...
				<div className='bg-gray-800 rounded-lg shadow-xl max-w-6xl w-full max-h-[90vh] overflow-y-auto'>
					<div className='flex justify-between items-center p-6 border-b border-gray-700'>
						<h2 className='text-2xl font-bold text-white'>
							{detailModal.type === 'user' && `User: ${detailModal.data?.username || ''}`}
							{detailModal.type === 'report' && `Report Details`}
							{detailModal.type === 'lobby' && `Lobby: ${detailModal.data?.roomId || ''}`}
						</h2>
						<button
					...
					<div className='p-6'>
						{detailLoading ? (
							...
							<>
								{detailModal.type === 'user' && <UserDetailContent user={detailModal.data} />}
								...
								{detailModal.type === 'lobby' && <LobbyDetailContent lobby={detailModal.data} />}
							</>
						)}
					</div>
				</div>
		...
		switch (activeView) {
			...
			case 'lobbies':
				return (
					<div className='bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-lg shadow-lg overflow-hidden'>
						<div className='p-6 border-b border-gray-200 dark:border-gray-700'>
							<h2 className='text-xl font-bold text-gray-900 dark:text-gray-100'>Active Lobbies</h2>
						</div>
						<div className='overflow-x-auto'>
							<table className='min-w-full divide-y divide-gray-200 dark:divide-gray-700'>
								<thead className='bg-gray-50/50 dark:bg-gray-900/50'>
			...
			default:
				return (
					<>
						<div className='bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-lg shadow-lg overflow-hidden'>
							<div className='p-6 border-b border-gray-200 dark:border-gray-700'>
								<h2 className='text-xl font-bold text-gray-900 dark:text-gray-100'>Chat Logs</h2>
							</div>
							<div className='overflow-x-auto'>
								<table className='min-w-full divide-y divide-gray-200 dark:divide-gray-700'>
									<thead className='bg-gray-50/50 dark:bg-gray-900/50'>
									...
									<tbody className='divide-y divide-gray-200 dark:divide-gray-700'>
										{paginateData(data.chats).map((chat, index) => (
											<tr
												key={index}
						...
						<PaginationControls totalItems={data.chats} />
					</>
				)
		}
	}
...
const UserDetailContent = ({ user }) => {
	...
	return (
		<div>
			<div className='border-b border-gray-700 mb-4'>
				<nav className='flex space-x-4'>
					<button
						...
						className={`py-2 px-4 ${
							activeTab === 'profile' ? 'border-b-2 border-blue-500 text-white' : 'text-gray-400'
						}`}>
						Profile
					</button>
					<button
					...
					<button
						...
						className={`py-2 px-4 ${
							activeTab === 'reports' ? 'border-b-2 border-blue-500 text-white' : 'text-gray-400'
						}`}>
						Reports
					</button>
					<button
			...
			{activeTab === 'chatHistory' && (
				<div className='mt-4'>
					<h3 className='text-lg font-medium text-white mb-2'>Recent Chat History</h3>
						...
						<div className='bg-gray-700 rounded p-4 max-h-96 overflow-y-auto'>
							{user.chatHistory.map((message, index) => (
								<div
									...
									<span className='text-gray-400 text-xs ml-2'>(Room: {message.roomId})</span>
								</div>
							))}
						</div>
					) : (
		...
		<div>
			<div className='grid grid-cols-1 md:grid-cols-2 gap-4 mb-6'>
				<div>
					...
					<p
						...
						onClick={() => handleViewDetails({ username: report.reportedUser }, 'user')}>
						{report.reportedUser}
					</p>
				</div>
				<div>
					...
					<p
						className='text-blue-400 hover:underline cursor-pointer'
						onClick={() => handleViewDetails({ username: report.reportedBy }, 'user')}>
						{report.reportedBy}
					</p>
				</div>
				<div>
			...
			{report.additionalComments && (
				<div className='mb-6'>
					...
					<div className='bg-gray-700 rounded p-4 text-white'>{report.additionalComments}</div>
				</div>
			)}
					...
					<div className='bg-gray-700 rounded p-4 max-h-64 overflow-y-auto'>
						{report.chatLogs.map((message, index) => (
							<div
								...
								<span
									className='text-blue-400 hover:underline cursor-pointer'
									onClick={() => handleViewDetails({ username: message.username }, 'user')}>
									{message.username}:
								</span>
		...
		<div>
			<div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
				...
				<div className='bg-gray-700/50 rounded-lg p-4'>
					<h3 className='text-lg font-medium text-white mb-3'>Lobby Information</h3>
					<div className='space-y-2'>
						...
						<div>
							...
							<span className='text-white ml-2 capitalize'>{lobby.selectCategory || 'random'}</span>
						</div>
					</div>
				</div>
				...
				{lobby.canvasData && (
					<div className='bg-gray-700/50 rounded-lg p-4'>
						<h3 className='text-lg font-medium text-white mb-3'>Current Drawing</h3>
						<div className='bg-white rounded overflow-hidden'>
							<img
							...
							<tbody>
								{lobby.players.map((player, index) => (
									<tr
										...
										<td className='p-2'>
											<button
												onClick={() => handleViewDetails({ username: player.username }, 'user')}
				...
				{lobby.messages && lobby.messages.length > 0 ? (
					<div className='bg-gray-800 rounded p-2 max-h-60 overflow-y-auto'>
						{lobby.messages.map((message, index) => (
							<div
								key={index}
								className='mb-2 hover:bg-gray-700 p-1 rounded'>
								<div className='flex justify-between'>
									<span
										className='font-bold text-blue-400 hover:underline cursor-pointer'
										onClick={() => handleViewDetails({ username: message.user }, 'user')}>
										{message.user}
									</span>
									<span className='text-xs text-gray-400'>
										{new Date(message.timestamp).toLocaleString()}
							...
							<tbody>
								{lobby.kickedUsers.map((kicked, index) => (
									<tr
										...
										<td className='p-2'>
											<button
												onClick={() => handleViewDetails({ username: kicked.username }, 'user')}
												className='text-blue-400 hover:underline'>
												View Profile
											</button>
										</td>
