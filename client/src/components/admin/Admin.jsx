    ...
    return (
      <div className='flex justify-between items-center px-6 py-4 bg-gray-50 dark:bg-gray-900/50'>
        <div className='text-sm text-gray-700 dark:text-gray-300'>
          ...
          {Math.min(currentData.page * ITEMS_PER_PAGE, currentData.total)} of {currentData.total}{' '}
          entries
        </div>
        <div className='flex gap-2'>
          <button
            onClick={() =>
              setUiState(prev => ({ ...prev, currentPage: Math.max(prev.currentPage - 1, 1) }))
            }
          ...
          <div className='flex items-center gap-2'>
            {[...Array(currentData.pages)].map((_, i) => (
              <button
                key={i}
                onClick={() => setUiState(prev => ({ ...prev, currentPage: i + 1 }))}
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setUiState(prev => ({ ...prev, error: `Failed to load ${uiState.activeView}` }))
      } finally {
  ...
  const handleSort = field => {
    setUiState(prev => ({
      ...
      sortConfig: {
        field,
        direction:
          prev.sortConfig?.field === field && prev.sortConfig?.direction === 'asc' ? 'desc' : 'asc',
      },
    }))
  }
  ...
  const handleUpdateStatus = async (reportId, newStatus) => {
    try {
      ...
      if (result.success && result.report) {
        ...
        setUiState(prev => ({
          ...prev,
          error: 'Failed to update report status: ' + (result.error || 'Unknown error'),
        }))
      }
    } catch (error) {
      setUiState(prev => ({
        ...prev,
        error: 'An error occurred while updating report status: ' + error.message,
      }))
    }
  }
    ...
    try {
      ...
      if (result?.success) {
        ...
        setData(prev => ({
          ...
          [type + 's']: {
            ...prev[type + 's'],
            items: prev[type + 's'].items.map(item => (item._id === id ? result[type] : item)),
          },
        }))
        ...
        <div className='max-w-7xl mx-auto space-y-6'>
          ...
          <div className='bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-lg shadow-lg p-6'>
            <div className='flex justify-between items-center'>
              ...
              {uiState.activeView !== 'overview' && (
                <button
                  onClick={() => setUiState(prev => ({ ...prev, activeView: 'overview' }))}
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4'>
            {[
              ...
              <button
                key={index}
                onClick={() => setUiState(prev => ({ ...prev, activeView: stat.view }))}
...
const ReportDetailContent = ({ report, isEditing, onSave, onChange, onStatusChange }) => {
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 gap-4 mb-6'>
        <div>
          ...
          {isEditing ? (
            ...
            <p className='text-blue-400 hover:underline cursor-pointer'>{report.reportedUser}</p>
          )}
        </div>
        <div>
          ...
          {isEditing ? (
            ...
            <p className='text-blue-400 hover:underline cursor-pointer'>{report.reportedBy}</p>
          )}
        </div>
        <div>
              ...
              <tbody>
                {lobby.players.map((player, index) => (
                  <tr
                    ...
                    <td className='p-2'>
                      <button
                        onClick={() => handleItemClick({ username: player.username }, 'user')}
              ...
              <tbody>
                {lobby.kickedUsers.map((kicked, index) => (
                  <tr
                    ...
                    <td className='p-2'>
                      <button
                        onClick={() => handleItemClick({ username: kicked.username }, 'user')}
                        className='text-blue-400 hover:underline'>
                        View Profile
                      </button>
                    </td>
