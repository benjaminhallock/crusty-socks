...
const ChatBox = ({
  ...
  const scrollToBottom = (force = false) => {
    ...
    if (
      ...
      (messages[messages.length - 1]?.username === user.username && isChatFocused)
    ) {
  ...
  useEffect(() => {
    if (shouldAutoScroll || messages[messages.length - 1]?.username === user.username) {
      scrollToBottom(false)
    }
  ...
  // initial loading of chat history, setup callback listeners
  useEffect(() => {
    ...
    const cleanupHistory = socketManager.onChatHistory(history => {
      ...
      if (Array.isArray(history)) {
        ...
        seenMessages.current = new Set(history.map(msg => msg._id).filter(Boolean))
        // Update messages state with complete history
        setMessages(history)
      }
    })
  ...
  const handleSubmit = e => {
    ...
    try {
      ...
      if (gameState === GAME_STATE.DRAWING) {
        if (user.username === currentDrawer && currentWord.includes(trimmedMessage)) {
        ...
        <div
          ...
          <div className='space-y-1'>
            {messages.map((msg, index) => (
              ...
              ${msg.username === user.username ? 'rounded-br-sm' : 'rounded-bl-sm'}
            `}>
                  <div
