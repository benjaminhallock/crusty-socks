...
const LobbySettings = ({ user }) => {
  ...
  const [gameState, setGameState] = useState({
    maxRounds: 3, // Number of rounds to play
    revealCharacters: 35, // Percentage of characters to reveal
    selectWord: 3, // Number of words to choose from
    selectCategory: 'random',
    playerLimit: 8, // Maximum players allowed
    roundTime: 60, // Time per round in seconds
    user: user,
    _id: user._id,
  })
  ...
  const handleCreateLobby = async () => {
    ...
    try {
      ...
      if (res.success && res.roomId) {
        setIsShowing(false)
        setTimeout(() => navigate(`/lobby/${res.roomId}`), 300)
      } else {
