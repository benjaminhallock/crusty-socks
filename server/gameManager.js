...
import { GAME_STATE as gs, SOCKET_EVENTS as se, WORD_LIST } from './constants.js'
...
class GameManager {
		...
		io.on('connection', socket => {
			...
			socket.on(se.CANVAS_UPDATE, data => {
				...
				this._canvasSaveTimer[roomId] = setTimeout(() => {
					Lobby.findOneAndUpdate({ roomId }, { canvasState: data.canvasState }, { new: true })
						.exec()
						.catch(console.error)
				}, 1000)
			})
	...
	async handleChatMessage(socket, { roomId, lobbyObjectId, message, username }) {
		...
		try {
			...
			if (lobby.gameState === gs.DRAWING && trim(message) === trim(lobby.currentWord)) {
				console.warn('delete this if it runs')
	...
	async startNextDraw(io, roomId) {
		...
		try {
			...
			if (lobby.currentRound > lobby.maxRounds && gameState === gs.FINISHED) {
	...
	async handleCanvasUpdate(socket, { roomId, canvasState }) {
		...
		// Only save to database when drawing stops or after debounce
