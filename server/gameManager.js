...
import { GAME_STATE as gs, SOCKET_EVENTS as se, trim, WORD_LIST } from './constants.js'
...
class GameManager {
  ...
  async handleConnectAck(socket, userData, roomId) {
    ...
    try {
      ...
      let existingPlayer = lobby.players.find(p => p.username === userData.username)
  ...
  async handleChatMessage(socket, { roomId, lobbyObjectId, message, username }) {
    ...
    if (filter.isProfane(message)) {
      return socket.emit('error', { message: 'Profanity is not allowed' })
    }

    if (message.length > 200) {
      return socket.emit('error', { message: 'Message cannot exceed 200 characters' })
    }
    ...
    try {
      ...
      // Emit to all clients in room immediately
      this.io.to(roomId).emit(se.CHAT_MESSAGE, {
        _id: savedChat._id,
        ...chatMessage,
      })
  ...
  async handleCheckWordGuess({ socket, roomId, guess: rawGuess, username }) {
    try {
      ...
      // Move normalization before usage
      const normalizedGuess = trim(rawGuess).toLowerCase()
      const normalizedWord = trim(lobby.currentWord).toLowerCase()

      const isCorrectGuess =
        normalizedGuess.startsWith(normalizedWord) &&
        normalizedGuess
          .replace(normalizedWord, '')
          .replace(/\W/g, '')
          .split('')
          .every(c => c === normalizedWord.slice(-1))
      ...
      if (!isCorrectGuess) {
        ...
        if (lobby.players.every(p => p.hasGuessedCorrect || p.username === lobby.currentDrawer)) {
    ...
    try {
	  ...
	  */
      ...
      setTimeout(async () => {
        ...
        const player = currentLobby.players.find(p => p.username === username)
        if (player && !player.connected) {
          currentLobby.players = currentLobby.players.filter(p => p.username !== username)
          await currentLobby.save()
          this.io.to(roomId).emit(se.GAME_STATE_UPDATE, { lobby: currentLobby })
        }
  ...
  async startNextDraw(io, roomId) {
    ...
    try {
      ...
      if (lobby.currentRound > lobby.maxRounds && lobby.gameState === gs.FINISHED) {
      ...
      const lobbyTimer = setTimeout(async () => {
        try {
          ...
          if (currentLobby && currentLobby.gameState === gs.DRAWING) {
            ...
            console.error(`[GM] ${roomId} - Timer expired but game state is not DRAWING`)
          }
        } catch (error) {
    ...
    try {
      ...
      const drawEndTimer = setTimeout(async () => {
        try {
          ...
          const allPlayersHaveDrawn = currentLobby.players.every(p => p.hasDrawn)
      ...
      Promise.all(
        lobby.players.map(async player => {
          try {
            ...
            await User.findOneAndUpdate({ username: player.username }, updateQuery, { new: true })
          } catch (error) {
            console.error(`Failed to update stats for player ${player.username}:`, error)
          }
        }),
      ).catch(error => {
  ...
  async handleRequestChatHistory(socket, { lobbyObjectId }) {
    try {
      ...
      try {
        ...
        // If ObjectId fails, try finding by roomId
        if (e.name === 'CastError') {
          const lobby = await Lobby.findOne({ roomId: lobbyObjectId })
          if (lobby) {
            messages = await Chat.find({
  ...
  async handleCanvasUpdate(socket, { roomId, canvasState }) {
    ...
    // Emit to all other clients immediately
    ...
    // Only save to database when drawing stops or after debounce (debounce is a term used for a delay)
    if (!canvasState.isDrawing) {
      // Clear any existing save timer
      this._cleanupCanvasTimers(roomId)
      ...
      }, 1000) // Reduced to 1 second since we're only saving when drawing stops
  ...
  findWordsForCategory(lobby) {
    ...
    if (lobby.selectCategory === 'random') {
      ...
      const categoryExists = Object.keys(WORD_LIST).includes(lobby.selectCategory)
      if (!categoryExists) {
        console.warn(`Category "${lobby.selectCategory}" not found. Using all words instead.`)
